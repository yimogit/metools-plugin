<template>
  ...
  >
    <ul
      class="layui-tab-title site-demo-title"
      style="margin-bottom:10px;"
    >
      <li
        :class="{'layui-this':tabType==1}"
        @click="switchTab(1)"
      >可逆加密/解密</li>
      <li
        :class="{'layui-this':tabType==2}"
        @click="switchTab(2)"
      >散列/哈希</li>
    </ul>
    <div
    ...
    >
      <div class="layui-form-item layui-form-text">
        <textarea
          v-model="model.beforeTxt"
          placeholder="加密/解密字符串..."
    ...
    >
      ...
      <div class="layui-form-item layui-form-text">
        <v-radio
          :value="model.currentType"
          :optionVals="hashTypes"
          @input="currentHandler"
        ></v-radio>
      </div>
      <div
        class="layui-form-item layui-form-text"
        v-if="hashPwdTypes.indexOf(model.currentType)>-1"
      >
        <input
          type="text"
          v-model="model.pwdTxt"
...
export default {
  ...
  methods: {
    loadData() {
      this.model =
        JSON.parse(localStorage.getItem("encrypt_model") || "null") ||
        this.model;
    },
    saveData() {
      localStorage.setItem("encrypt_model", JSON.stringify(this.model));
    },
    fetchData() {
      this.tabType = this.$route.query.tab || 1;
      if (this.tabType == 1 && !this.model.currentType) {
        this.model.currentType = this.encryptTypes[0];
      } else if (!this.model.currentType) {
